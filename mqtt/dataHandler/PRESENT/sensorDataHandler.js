...
const cryptoUtils = require('../../../crypto/PRESENTCryptoUtils/cryptoUtils');
...
async function handleSensorData(deviceId, data) {
  ...
  const { device_id, ciphertext, tag, iv, encryption_type } = data;
  ...
  if (!device_id || !ciphertext || !tag || !iv || encryption_type !== 'present-cbc') {
  ...
  const decryptionResult = await cryptoUtils.decryptData(ciphertext, tag, iv, sharedSecret);
  if (!decryptionResult || decryptionResult.status !== 'success') {
    logError.error(`Decryption failed`, { 
      ...
      message: decryptionResult ? decryptionResult.message : 'unknown'
    });
  ...
  try {
    const sensorData = new SensorData({
      ...
      encryption_type: decrypted.encryption_type || 'present-cbc',
      nonce: iv, // Store IV as nonce for consistency
      tag: tag,
    ...
    await EncryptedSensorData.findOneAndUpdate(
      { device_id, encrypted_data: ciphertext, nonce: iv },
      {
        is_decrypted: true,
