...
async function handleSensorData(deviceId, data) {
  ...
  const { device_id, ciphertext, tag, nonce, encryption_type } = data;
  ...
  if (!device_id || !ciphertext || !tag || !nonce || encryption_type !== 'chachapoly') {
  ...
  try {
    const sensorData = new SensorData({
      ...
      encryption_type: decrypted.encryption_type || 'chachapoly',
      nonce: nonce, 
      tag: tag,
    ...
    await EncryptedSensorData.findOneAndUpdate(
      { device_id, encrypted_data: ciphertext, nonce: nonce },
      {
        is_decrypted: true,
